...
impl View for GanttChart {
    ...
    fn render(&mut self, ui: &mut egui::Ui, app: &mut ApplicationContext) {
        ...
        let min_start = app.filtred_jobs.iter().map(|job| job.scheduled_start).min().unwrap_or(0);
        let max_end = app.filtred_jobs.iter().map(|job| job.scheduled_start + job.walltime).max().unwrap_or(0);
